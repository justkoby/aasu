  ...
  return (
    <header className={`main-header ${isScrolled ? 'scrolled' : ''}`}>
        ...
        {isMobileMenuOpen && (
          ...
          >
             <div className="mobile-header">
                ...
                <X className="close-mobile" size={32} onClick={() => setIsMobileMenuOpen(false)} />
             </div>
             <div className="mobile-links">
                {navItems.map((item, i) => (
                  ...
                  >
                    {item.label}
                    {item.hasMega && <ChevronDown size={18} />}
                  </motion.a>
                ))}
             </div>
      ...
      <style dangerouslySetInnerHTML={{ __html: `
        ...
        .nav-bar.glass {
          background: transparent; /* Background now handled by main-header */
          border-bottom: none;
        }
